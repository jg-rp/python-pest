...
def unescape_string(value: str, token: Pair, quote: str) -> str:
...
def _decode_escape_sequence(  # noqa: PLR0911
    value: str, index: int, token: Pair, quote: str
) -> Tuple[str, int]:
    try:
        ch = value[index]
    ...
    raise JSONPathSyntaxError("unknown escape sequence", token=token)


def _decode_hex_char(value: str, index: int, token: Pair) -> Tuple[int, int]:
    length = len(value)

    if index + 4 >= length:
        raise JSONPathSyntaxError("incomplete escape sequence", token=token)

    index += 1  # move past 'u'
    codepoint = _parse_hex_digits(value[index : index + 4], token)

    if _is_low_surrogate(codepoint):
        raise JSONPathSyntaxError("unexpected low surrogate at index", token=token)
    ...
    if _is_high_surrogate(codepoint):
        ...
        if not (
            ...
            raise JSONPathSyntaxError(
                "incomplete escape sequence at index",
                token=token,
            )
...
def _parse_hex_digits(digits: str, token: Pair) -> int:
    codepoint = 0
...
def _string_from_codepoint(codepoint: int, token: Pair) -> str:
    if codepoint <= 0x1F:
